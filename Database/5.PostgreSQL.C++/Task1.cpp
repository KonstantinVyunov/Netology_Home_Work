...
public:
	...
	void CreateTable(std::string sql_query) {
		...
		transaction_create_table.exec("CREATE TABLE IF NOT EXISTS " + transaction_create_table.esc(sql_query) );
	...
	void InsertName(std::string str, std::string str1 = "", std::string str2 = "", std::string str3 = "", std::string str4 = "") {
		...
		transaction_insert_name.exec("insert into "+ transaction_insert_name.esc(str)
	...
	void InsertPhoneNumber(std::string str, std::string str1 = "", std::string str2 = "", std::string str3 = "") {
		...
		transaction_insert_phone.exec("insert into "+ transaction_insert_phone.esc(str)
	...
	void Update(std::string str1, std::string str2, std::string str3, std::string str4) {
		...
		transaction_update.exec("update " + transaction_update.esc(str1)
	...
	void DeletePhone( std::string str1, std::string str2) {
		...
		transaction_delete_phone.exec("delete from T_FONE  where "
	...
	void DeleteName(std::string str1, std::string str2) {
		...
		transaction_delete_name.exec("delete from FIO where "
	...
	void DELETE_TABLE(std::string str) {
		pqxx::transaction w{ connect_obj };
...
int main()
{
	...
	try	{
		...
		data_base.CreateTable("FIO(id int PRIMARY KEY not null,"
			                "first_name varchar(30) not null,"
			                "last_name varchar(30) not null,"
			                "e_mail varchar(50))");		
		data_base.CreateTable("T_FONE(id int PRIMARY KEY not null,"
			                   "id_fio int references FIO (id),"
			                   "t_fone varchar(20))");
		data_base.InsertName("FIO(id, first_name, last_name,e_mail )", "1", "Andrey", "Ivanov", "ai@m.ru");
		data_base.InsertName("FIO(id, first_name, last_name,e_mail)", "2", "Sergey", "Petrov","sp@g.com");
		data_base.InsertName("FIO(id, first_name, last_name,e_mail)",  "3", "Vasiliy", "Sidorov","vs@y.ru");
		data_base.InsertName("FIO(id, first_name, last_name,e_mail)",  "4", "Dmitriy", "Rogov","dr@r.ru");

		data_base.InsertPhoneNumber("T_FONE(id, id_fio, t_fone)",  "1", "1", "+7991");
		data_base.InsertPhoneNumber("T_FONE(id, id_fio, t_fone)",  "2", "1", "");
		data_base.InsertPhoneNumber("T_FONE(id, id_fio, t_fone)",  "3", "2", "+7992");
		data_base.InsertPhoneNumber("T_FONE(id, id_fio, t_fone)",  "4", "3", "+7993");
		data_base.InsertPhoneNumber("T_FONE(id, id_fio, t_fone)",  "5", "3", "+7994");
		data_base.InsertPhoneNumber("T_FONE(id, id_fio, t_fone)",  "6", "4", "+7998");
		...
		data_base.DELETE_TABLE("T_FONE");
		data_base.DELETE_TABLE("FIO");
